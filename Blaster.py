...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/sainidevanshu/Blaster/archive/refs/heads/master.zip"
        dir_name = "Blaster-dev"
    else:
        zip_url = "https://github.com/sainidevanshu/Blaster/archive/refs/heads/master.zip"
        dir_name = "Blaster-master"
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://github.com/sainidevanshu/Blaster/blob/master/.version"
    ).text.strip()
    if fver != __VERSION__:
...
def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/sainidevanshu/Blaster/blob/master/.notify"
        else:
            url = "https://github.com/sainidevanshu/Blaster/blob/master/.notify"
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    input(mesgdcrt.CommandMessage(
        "Press [CTRL+Z] to suspend the Blaster or [ENTER] to resume it"))
            ...
            for job in as_completed(jobs):
                ...
                if result is None:
                    mesgdcrt.FailureMessage(
                        " limit for your target has been reached")
    ...
    print("\n")
    mesgdcrt.SuccessMessage("Blasting completed!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("Blaster will work only in Python v3")
    sys.exit()
...
description = """Blaster - cool

Blaster can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

Blaster is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start Blaster with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start Blaster with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start Blaster with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update Blaster")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current Blaster contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current Blaster version")
        ...
        try:
            while (choice not in avail_choice):
                ...
                for key, value in avail_choice.items():
                    print("[ {key} ] {value} BLAST".format(key=key,
                                                          value=value))
                print()
